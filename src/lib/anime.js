...
function spring(string, duration) {
	...
	function solver(t) {
		const progress = duration ? (duration * t) / 1000 : t;
		if (zeta < 1) {
			progress =
...
function getPolylineLength(el) {
	...
	const totalLength = 0;
	let previousPos;
	for (const i = 0; i < points.numberOfItems; i++) {
...
function flattenKeyframes(keyframes) {
	...
	for (const i = 0; i < propertyNames.length; i++) loop(i);
	return properties;
}
...
function normalizeTweenValues(tween, animatable) {
	...
	for (const p in tween) {
		const value = getFunctionValue(tween[p], animatable);
		if (is.arr(value)) {
...
const engine = (function () {
	...
	function step(t) {
		...
		const activeInstancesLength = activeInstances.length;
		const i = 0;
...
function anime(params) {
	...
	const promise = makePromise(instance);
	...
	function syncInstanceChildren(time) {
		if (!instance.reversePlayback) {
			...
			for (const i$1 = childrenLength; i$1--; ) {
	...
	function setAnimationsProgress(insTime) {
		...
		while (i < animationsLength) {
			...
			const tween = tweens[tweenLength];
			...
			const progress = void 0;
			for (const n = 0; n < toNumbersLength; n++) {
				const value = void 0;
			...
			if (!stringsLength) {
				...
				progress = strings[0];
				for (const s = 0; s < stringsLength; s++) {
					const a = strings[s];
					const b = strings[s + 1];
					const n$1 = numbers[s];
					if (!isNaN(n$1)) {
...
function removeTargetsFromAnimations(targetsArray, animations) {
	for (const a = animations.length; a--; ) {
...
function removeTargetsFromActiveInstances(targets) {
	const targetsArray = parseTargets(targets);
	for (const i = activeInstances.length; i--; ) {
...
function stagger(val, params) {
	...
	return function (el, i, t) {
		...
		if (!values.length) {
			for (const index = 0; index < t; index++) {
				if (!grid) {
